...
export default function Navbar() {
  ...
  return (<>
    ...
    <nav className={`flex fixed z-10 w-full ${headerClr ? 'bg-white shadow-md' : ''}  top-0 bg-transparent md:px-16 px-8 py-8 h-[100px] items-center`}>
      ...
      <div className={`md:hidden absolute top-0 left-0  h-screen w-screen z-50 bg-gradient-to-bl to-[#1B4582] from-[#BBC5D1] transform ${open ? "-translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out  `}>
        <div className="flex items-center justify-center mx-auto h-24" onClick={() => setOpen(false)}> {/*logo container*/}
          <Link href="/">
      ...
      <div className="w-full flex justify-between items-center">
        ...
        <div className="z-50 flex relative w-8 h-6 flex-col gap-3 items-center md:hidden" onClick={() => {
          setOpen(!open)
        }}>
          {/* hamburger button */}
          ...
          <span className={`h-[3px] w-full bg-black rounded-lg transform transition duration-300 ease-in-out ${open ? "-rotate-45 -translate-y-2.5 mt-1 bg-white" : ""}`} />
        </div>
